...
class filamentsensorngPlugin(octoprint.plugin.StartupPlugin,
    ...
    @property
    def sensor_triggered_at(self):
        return int(self.triggered_at)
        ...
        if self.no_filament():
            ...
            if self.confirmations <= self.filamentsensorngPlugin_confirmations_tracking:
                ...
                if self.send_gcode_only_once 
                    if self.sensor_triggered_at() > int(time()) - 300:
                        self._logger.info("Sensor callback executed less than 5 minutes ago, not executing again")
                        return
                    else
                        self.triggered_at = int(time())
    ...
    def get_update_information(self):
        return dict(
            octoprint_filamentsensorng=dict(
                ...
                # update method: pip
                pip="https://github.com/Red_M/Octoprint-Filament-Sensor-ng/archive/{target_version}.zip"
            )
        )
